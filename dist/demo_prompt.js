...
import { CLIENT_NAME, CLIENT_VERSION, MCP_SERVER_URL, MODEL, OPENAI_API_KEY, OPENAI_BASE_URL, } from "./constants";
import { appendScreenshot, convertMcpToolsToOpenAiFormat, ensureConnection, handleNonStreamingConversation, handleStreamingConversation, } from "./utils/utils";
...
export const prompt = async (payload) => {
    ...
    try {
        ...
        const openAiTools = convertMcpToolsToOpenAiFormat(availableTools);
        ...
        if (isStreaming) {
            return new ReadableStream({
                async start(controller) {
                    try {
                        await handleStreamingConversation(messages, openAiTools, controller, openAI, client, MODEL);
                    }
                    catch (error) {
            ...
            let response = await handleNonStreamingConversation(messages, openAiTools, openAI, client, MODEL);
...
//# sourceMappingURL=demo_prompt.js.map